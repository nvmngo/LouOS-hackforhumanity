...
export default function OnlineReportField({field,value,values,onChange}){
  ...
  if(field.type==='conditionalCount'){
    const checked=values?.[field.yesId]==='Yes';
    const toggle=()=>{onChange(field.yesId,checked?'No':'Yes');if(checked)onChange(field.id,'');};
    return <fieldset className="online-field online-children"><legend>{field.label}</legend><div><label><input type="checkbox" checked={checked} onChange={toggle}/><span>If yes, how many?</span></label><input aria-label="Number of children" type="number" min="0" inputMode="numeric" disabled={!checked} value={value||''} onChange={e=>onChange(field.id,e.target.value)}/></div></fieldset>;
  }
