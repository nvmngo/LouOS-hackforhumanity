...
export function approveSuggestion(draft,item){const next=clone(draft);if(item.area==='Main Need')next.mainSupportNeed=item.text;else if(['Housing','Financial','Legal'].includes(item.area)){const priority=item.area==='Housing'?'High':item.area==='Financial'?'Medium':'Low';const existing=next.problems.find(x=>x.category===item.area);next.problems=upsertProblem(next.problems,item.area,priority,existing?`${existing.description} ${item.text}`:item.text);}next.keyInformation=addFact(next.keyInformation,item.text);return regenerate(next);}