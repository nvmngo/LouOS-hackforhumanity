...
import {currentEmployee,employeePortal,employeeToken,isPrototypeEmployee} from '@/lib/employeeSession';
...
export default function EmployeeReferrals(){
  ...
  useEffect(()=>{currentEmployee().then(setMe).catch(()=>{});},[]);
  ...
  useEffect(()=>{
    ...
    base44.functions.invoke('suggestReferralCategory',{caseId:data.id,employeeToken:employeeToken(),
  ...
  const persist=async(nextReport,historyEntry)=>{
    ...
    const reportVersion=(data.reportVersion||1)+1;
    if(isPrototypeEmployee())await employeePortal('saveCaseReport',{caseId:data.id,caseReport:nextReport,reportHistory:history,reportVersion});
    else await base44.entities.ClientSubmission.update(data.id,{case_report:nextReport,report_history:history,report_version:reportVersion});
  ...
  const findServices=async()=>{
    ...
    try{
      const response=await base44.functions.invoke('recommendOrganizations',{caseId:data.id,employeeToken:employeeToken(),
        category,
        primaryNeed:report.supportNeeds.primaryNeed,
        secondaryNeeds:report.supportNeeds.secondaryNeeds,
  ...
  const runBrowse=async query=>{
    ...
    try{
      const response=await base44.functions.invoke('browseServiceDirectory',{caseId:data.id,employeeToken:employeeToken(),
        query,
        secondaryNeeds:report.supportNeeds.secondaryNeeds,
        age:report.clientInformation.age,
        dependantsPresent:dependantsPresent(report.clientInformation.dependants)
  ...
  const draftEmail=async(record)=>{
    ...
    try{
      const response=await base44.functions.invoke('draftReferralEmail',{caseId:data.id,employeeToken:employeeToken(),
  ...
  const send=async()=>{
    ...
    try{
      await base44.functions.invoke('sendReferralEmail',{caseId:data.id,employeeToken:employeeToken(),to:selectedOrg.contact,subject:emailSubject,body:emailBody});
  ...
  return <main className="mvp-main employee-referrals">
    <ReturnButton variant="inline" to={`/employee/cases/${caseId}/decision`} label="Back to the external support step"/>
    ...
    {step===5&&<section className="referral-step-panel referral-done"><ShieldCheck size={34}/><h2>Referral sent</h2><p>{selectedOrg.name} has been recorded as the referral for this case.</p>
      <div className="phase-actions"><span/><Link className="mvp-btn" to={`/employee/cases/${caseId}/final?external=yes`}>Continue to final report <ArrowRight size={17}/></Link></div>
    </section>}
    </>}
  </main>;
}
