...
const supportCategory: Record<string,string> = {Housing:'Accommodation','Domestic or family violence':'Domestic / family violence support','Domestic / Family Violence':'Domestic / family violence support',Financial:'Financial assistance',Legal:'Legal support','Health or wellbeing':'Health','Health / Wellbeing':'Health',Employment:'Employment','Family or children':'Family / child support','Family / Children':'Family / child support','Social support':'Social support','Social Support':'Social support',Safety:'Other',Other:'Other'};
...
const createCaseReport = (candidate: any, data: any, source: string, urgency: string, categories: string[]) => {
  ...
  const urgent = text(data.urgentAttention);
  const secondaryNeeds = categories.map(category => supportCategory[category] || category);
  const safetyConcerns = [
    ...(secondaryNeeds.includes('Domestic / family violence support') ? ['Violence / threats'] : []),
  ...
  const fallback: any = {
    caseOverview:{caseId:'',openedDate:text(data.date),assignedSpecialist:'',status:'New',urgency,preferredLanguage:text(source === 'paper' ? data.preferred_language : data.preferredLanguage),preferredContactMethod:text(data.safeContactPreference || data.safe_contact)},
    clientInformation:{fullName:text(source === 'paper' ? data.client_name : data.fullName),preferredName:text(source === 'paper' ? data.preferred_name : data.preferredName),age:text(data.age),pronouns:text(data.pronouns),phone:contact.includes('@')?'':contact,email:contact.includes('@')?contact:text(data.email),dependants:text(source === 'paper' ? data.dependants : data.childrenDependants),accommodation:text(source === 'paper' ? data.accommodation : data.currentAccommodation),preferredContactMethod:text(data.safeContactPreference || data.safe_contact),safeToContact:text(data.safeToday),contactInstructions:text(data.safeContactPreference || data.safe_contact)},
    presentingSituation:{summary:text(source === 'paper' ? data.summary : [data.reasonToday,data.recentEvents].filter(Boolean).join(' ')),recentChanges:text(source === 'paper' ? '' : [data.recentEvents,data.stayDuration].filter(Boolean).join(' '))},
    safety:{level:safetyLevel,concerns:safetyConcerns,notes:urgent,sources:safetyConcerns.length?['Client reported']:[]},
    supportNeeds:{primaryNeed:text(source === 'paper' ? data.main_need : data.helpToday)||'General support',secondaryNeeds,clientPriority:text(source === 'paper' ? data.main_need : data.helpToday)},
    background:{previousSupport:'',existingServices:'',supportNetwork:'',financialSituation:'',employmentSituation:'',healthInformation:'',legalMatters:'',previousIncidents:'',other:text(source === 'paper' ? (data.key_information||[]).join(' ') : data.otherFacts)},
    clientGoals:{immediateGoal:text(source === 'paper' ? data.main_need : data.helpToday),longerTermGoal:'',preferredSupport:'',concerns:'',preferences:'',declinedSupport:''},
...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    const urgentAttention = source === 'online' ? text(data.urgentAttention) : '';
    const hasUrgentDetail = Boolean(urgentAttention) && !/^(no|none|nothing|n\/a|not applicable)\b/i.test(urgentAttention);
    ...
    const highSignal = safeToday === 'unsure' || hasUrgentDetail || priorities.some(value => /high/i.test(value));
    ...
    const ranked = specialistsWithCongestion.filter((item: any) => (
      ...
      isAvailableToday(item.available_days, today)
    )).map((item: any) => {
      const expertiseScore = (item.expertise || []).reduce((score: number, skill: string) => score + (expertiseMatches(skill, searchable) ? 24 : 0), 0);
      ...
      return { item, score: Math.round(expertiseScore + languageScore + availabilityScore + capacityScore + experienceScore) };
