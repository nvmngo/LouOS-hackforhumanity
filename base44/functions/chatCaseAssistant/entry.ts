...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    if (!['interview','quick_notes','online_report'].includes(operation)) return Response.json({ error: 'Invalid assistant operation.' }, { status: 400 });
    const inputLimit = operation === 'online_report' ? 10000 : 4000;
